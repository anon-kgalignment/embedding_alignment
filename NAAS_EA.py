...
def parse_args():
    parser = argparse.ArgumentParser(description="Entity Alignment Training Script")
...
def main():
    args = parse_args()  # You define this to handle command-line arguments

    # Replace all hardcoded paths with args.source_dir, args.target_dir, etc.
    # Move all processing logic (embedding loading, alignment, training, evaluation) inside here
    ...
    def loss_fn(S_aligned, T_aligned, S_shared, T_shared, S_train_tensor, T_train_tensor, w1, w2, w3, w4, return_details=False):
        ...
        def contrastive_loss(S_shared, T_shared, margin=1.0, k=20, batch_size=512):
            ...
            for start in range(0, batch_size_total, batch_size):
                ...
                sim_matrix = F.cosine_similarity(S_batch.unsqueeze(1), T_shared.unsqueeze(0), dim=2)  # shape: [B, N]
                
                pos = torch.diagonal(sim_matrix[:, start:end]) if end - start == batch_size else sim_matrix[:, start:end].diagonal()
        ...
        contrastive_loss_val, pos_mean, mean_hard_neg = contrastive_loss(S_shared, T_shared)
        directional_loss = cosine_similarity_loss(S_train_tensor, S_aligned) + cosine_similarity_loss(T_train_tensor, T_aligned)
        ...
        if weight_sum == 0:
            total_loss = structure_loss + contrastive_loss_val + directional_loss + magnitude_loss
        else:
            total_loss = (w1 * structure_loss + w2 * contrastive_loss_val + w3 * directional_loss + w4 * magnitude_loss)
    ...
    def train_alignment_model(S_train, T_train, S_val, T_val, input_dim, hidden_dim, epochs, lr,
        ...
        for epoch in range(epochs):
            ...
            base_model.eval()
            with torch.no_grad():
                S_val_aligned, _ = base_model(S_val_tensor)#S_val_neighbors_tensor
                T_val_aligned, _ = base_model(T_val_tensor)#S_val_neighbors_tensor
            ...
            model_path = os.path.join(os.path.dirname(log_path), f"alignment_model_epoch{epoch+1}.pt")
        ...
        update_bn(DataLoader(TensorDataset(S_train_tensor), batch_size=64), swa_model)

        final_swa_model_path = os.path.join(os.path.dirname(log_path), "final_swa_model.pt")
    ...
    swa_model.load_state_dict(torch.load(best_swa_model_path, map_location="cpu"))
    swa_model.eval()
    ...
    torch.save(swa_model.state_dict(), os.path.join(final_save_dir, "final_alignment_model.pt"))
    ...
    with open(os.path.join(final_save_dir, "final_metadata.json"), "w") as f:
        json.dump({
            ...
            "Hits@10": hits_at_k[2],
            "Hits@50": hits_at_k[3],
            "best_trial_folder": best_trial_dir  # ✅ Add this line
        }, f, indent=4)
