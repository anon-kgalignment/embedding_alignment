...
def parse_args():
    parser = argparse.ArgumentParser(description="Neural Alignment Pipeline for KGE")
    parser.add_argument('--dir1', type=str, required=True, help="Path to the first dataset folder")
    parser.add_argument('--dir2', type=str, required=True, help="Path to the second dataset folder")
    parser.add_argument('--alignment_dir', type=str, required=True, help="Path to folder with alignment files (.nt, .ttl, etc.)")
    parser.add_argument('--test_triples', type=str, required=True, help="Path to the test triples file")
...
def main(args):
    ...
    save_dir = os.path.join(output_dir, "aligned_embeddings")  # define this BEFORE using it
    ...
    model_path_1, entity_to_id_1, relation_to_id_1 = extract_files_from_directory(directory_1)
    model_path_2, entity_to_id_2, relation_to_id_2 = extract_files_from_directory(directory_2)
    entity_embeddings_df1, relation_embeddings_df1=load_embeddings(model_path_1, entity_to_id_1, relation_to_id_1)
    entity_embeddings_df2, relation_embeddings_df2=load_embeddings(model_path_2, entity_to_id_2, relation_to_id_2)
    ...
    def build_alignment_dict(folder_path):
        ...
        if not os.listdir(folder_path):  # Folder is empty
            logging.warning(f"Alignment folder '{folder_path}' is empty. Skipping processing.")
            return alignment_dict
                ...
                if extension in ['.nt', '.ttl']:
                    ...
                    for subj, pred, obj in g:
                        ...
                        if "sameAs" in pred:
                            ...
                            logging.warning(f"Skipping triple with unexpected predicate: {subj} {pred} {obj}")
    ...
    relation_embeddings1=remove_brackets_from_indices(relation_embeddings_df1)
    relation_embeddings2=remove_brackets_from_indices(relation_embeddings_df2)
    ...
    S_train, T_train, S_test, T_test= create_train_test_matrices(cleaned_alignment_dict, entity_embeddings1, entity_embeddings2, test_size=0.1)
    ...
    S_train_normalized, S_train_mean, S_train_scale = normalize_and_scale(S_train)
    T_train_normalized, T_train_mean, T_train_scale = normalize_and_scale(T_train)


    S_test = pd.DataFrame(np.array(S_test))
    T_test = pd.DataFrame(np.array(T_test))
    S_test_normalized, _, _ = normalize_and_scale(S_test, reference_data=S_train)
    T_test_normalized, _, _ = normalize_and_scale(T_test, reference_data=T_train)
    S_test_normalized = S_test_normalized.to_numpy() if hasattr(S_test_normalized, 'to_numpy') else S_test_normalized
    T_test_normalized = T_test_normalized.to_numpy() if hasattr(T_test_normalized, 'to_numpy') else T_test_normalized
    ...
    class SharedSpaceAlignmentNN(nn.Module):
        ...
        def forward(self, entities):
            ...
            aligned_embeddings = (1 - alpha_scaled) * entities + alpha_scaled * shared_space
    ...
    def train_alignment_model(S_train, T_train, input_dim, hidden_dim, epochs, lr, w1, w2, w3, w4):
        ...
        for _ in range(epochs):
            ...
            loss = loss_fn(S_aligned, T_aligned, S_shared, T_shared, S_train_tensor, T_train_tensor, w1, w2, w3, w4)
    ...
    def evaluate_model(kge_model, test_triples):
        ...
        performance = evaluate_link_prediction_performance(kge_model, test_triples, er_vocab, re_vocab)
    ...
    merged_embeddings_full = pd.concat([entity_embeddings1, entity_embeddings2])

    # Check explicitly for duplicates before removing
    duplicates = merged_embeddings_full.index[merged_embeddings_full.index.duplicated(keep=False)]

    # Remove duplicates, clearly keeping the first occurrence only
    merged_embeddings_full_no_duplicates = merged_embeddings_full[~merged_embeddings_full.index.duplicated(keep='first')]
    ...
    merged_embeddings_normalized, _, _ = normalize_and_scale(sorted_merged_embeddings_full, reference_data=S_train)
    merged_embeddings_tensor = torch.tensor(merged_embeddings_normalized.values, dtype=torch.float32)
    ...
    sorted_merged_relation_full = sorted_merged_relation_full[~sorted_merged_relation_full.index.duplicated(keep='first')]
    merged_relation_normalized, _, _ = normalize_and_scale(sorted_merged_relation_full, reference_data=S_train)
    relation_tensor = torch.tensor(merged_relation_normalized.values, dtype=torch.float32)
    ...
    relation_save_path = os.path.join(save_dir, "merged_relation_embeddings.pt")

    # Convert to PyTorch tensors (if not already)
    merged_entity_tensor = torch.tensor(merged_embeddings_tensor.numpy(), dtype=torch.float32)
    relation_tensor = torch.tensor(merged_relation_normalized.values, dtype=torch.float32)
    ...
    # **Load Test Triples**
    test_triples = pd.read_csv(test_triples_path, sep="\s+", header=None, names=['subject', 'relation', 'object'], dtype=str).values.tolist()[:1000]

    if not os.path.exists(previous_model_path):
        raise FileNotFoundError(f"Expected previous model at {previous_model_path} not found.")
    if not os.path.exists(new_model_path):
        raise FileNotFoundError(f"Expected new model at {new_model_path} not found.")
    ...
    for idx, (w1, w2, w3, w4) in enumerate(product(*param_grid.values()), 1):
        ...
        with torch.no_grad():
            aligned_entity_embeddings, _ = alignment_model(merged_embeddings_tensor.clone().detach())
            aligned_entity_embeddings = aligned_entity_embeddings.numpy()

            aligned_relation_embeddings, _ = alignment_model(relation_tensor.clone().detach())
            aligned_relation_embeddings = aligned_relation_embeddings.numpy()
        

        # **Step 3: Train New KGE Model on Aligned Embeddings**
        kge_model = initialize_models_and_update_embeddings(aligned_entity_embeddings, aligned_relation_embeddings, new_model_folder)

        #  Save the model inside the folder (Torch format)
        torch.save(kge_model.state_dict(), os.path.join(new_model_folder, "model.pt"))
        ...
        if new_mrr > best_mrr:
            best_mrr = new_mrr
            best_grid_params = [w1, w2, w3, w4]  #  Store best Grid Search parameters
            print(f" New model (MRR={new_mrr}) is better. Replacing previous model.")
            ...
            print(f" New model (MRR={new_mrr}) is NOT better. Keeping old model.")
    ...
    print(f" Grid Search took {grid_search_duration:.2f} seconds ({grid_search_duration / 60:.2f} minutes)")
    ...
    best_bayesian_model = train_alignment_model(S_train, T_train, input_dim, 256, 500, 0.001, w1, w2, w3, w4)

    # **Apply Alignment to Full Dataset**
    with torch.no_grad():
        aligned_embeddings_bayesian, _ = best_bayesian_model(merged_embeddings_tensor)
    ...
    final_bayesian_kge  = initialize_models_and_update_embeddings(aligned_entity_embeddings, aligned_relation_embeddings, new_model_folder)
    ...
    torch.save(final_bayesian_kge.state_dict(), os.path.join(new_model_folder, "model.pt"))
    ...
    full_test_triples = pd.read_csv(test_triples_path, sep="\s+", header=None, names=['subject', 'relation', 'object'], dtype=str).values.tolist()
    ...
    if best_mrr_bayesian > best_mrr:
        print(f" Bayesian Optimization found a better model (MRR={best_mrr_bayesian})! Replacing Grid Search model.")
